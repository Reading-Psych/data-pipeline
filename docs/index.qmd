...
If your participants originally signed up through SONA, you [[**should**]{.underline}]{style="color: orange"} select SONA (even if they have been passed to REDCap before arriving at Gorilla). This is because you ultimately want participants to be awarded SONA credits on completion of the experiment on Gorilla, and gorilla needs to know that they were recruited through SONA to do this.

![Finding correct URL for your gorilla experiment](images/5_3.png){#fig-5.3}

Now that Gorilla has generated a link that will take a participant to the Gorilla study, we can use this (or a slightly modified version of it) in REDCap to direct the participant to Gorilla after they have completed the last task in REDCap (see [6 Passing participant ID from REDCap to Gorilla]).

(NB do not use the generic URL that displays when you are viewing your survey/task (the one that will look something like this: [https://app.gorilla.sc/task/6878668]{style="color:orange"}). This will not work.)

Instead, you want to use the link circled in Figure 9 but changed slightly (manually) before you paste it into REDCap:

https://research.sc/participant/login/dynamic/3A3E0826-CF98-4846-AC1D-89E635E55B00?external_id=[%SURVEY_CODE%]{style="background-color:yellow"}  ... becomes:

https://research.sc/participant/login/dynamic/3A3E0826-CF98-4846-AC1D-89E635E55B00?external_id=[\[ppt_id[\]]{.underline}]{style="background-color:yellow"}

[(Remember not to directly use our example links/URLs as they will not work, you need to use your own project's URLs)]{style="color:red"}

You have to do this manually, e.g. in Word or Notepad, to change %SURVEY_CODE% to \[ppt_id\]. This is the link that we will need to paste into REDCap (see [6 Passing participant ID from REDCap to Gorilla]).

If you want to know more about why you do this and what the links are doing go to: [10.7 Recognising URLS from SONA, Gorilla and REDCap]
...
Paste the edited Gorilla URL (the one that looks like our example: https://research.sc/participant/login/dynamic/3A3E0826-CF98-4846-AC1D-89E635E55B00[?external_id=\[ppt_id\]]{style="background-color:yellow"}) into the 'redirect to a URL' setting under 'Survey Termination Options'. It is quite a long way down in the survey settings page, so keep scrolling down.
...
::: {style="color: brown"}
https://rdg-psychology.sona-systems.com/webstudy_credit.aspx?experiment_id=2532&credit_token=c4dbc01a4b5a4dd8ad3042e00683c855[&survey_code=\[sona_id:value\]]{style="background-color:yellow"}
:::
...
However when using this URL with gorilla, you need to delete the URL parameter '[&survey_code=\[sona_id:value\]]{style="color:brown"}' as gorilla has built in functionality for passing the survey code back to SONA and so this is not needed:

[https://rdg-psychology.sona-systems.com/webstudy_credit.aspx?experiment_id=2532&credit_token=c4dbc01a4b5a4dd8ad3042e00683c855]{style="color: brown"}
...
Once again, you will need to add the SONA completion link (see [7 Passing participant from Gorilla back to SONA for automatic crediting] for where to find this) into the 'onward URL' section of the reject node, in the same way you did with the finish node previously.
...
You are now ready for your experiment to go live. [**N.B. Make sure you change your gorilla 'reject' node back to a 'finish' node once you have completed testing, before you begin to collect real participant data. If you do not, any participant data you collect will be deleted by gorilla.**]{style="color:red"}
...
[?]{style="color:red"}[PROLIFIC_PID]{style="background-color:yellow"}={{%PROLIFIC_PID%}}&[STUDY_ID]{style="background-color:yellow"}={{%STUDY_ID%}}&[SESSION_ID]{style="background-color"}={{%SESSION_ID%}}
...
[?]{style="background-color:red"}prolific_pid={{%PROLIFIC_PID%}}&study_id={{%STUDY_ID%}}&session_id={{%SESSION_ID%}}
...
You can then need to combine the Public Survey URL from REDCap with the suffix that Prolific generated for you. To do this take the [**Public Survey URL**]{style="color:teal"}, then add an ampersand ([**&**]{style="color: purple"}) symbol, and finally, the [**suffix generated by Prolific**]{style="color:brown"}. For example:

[**https://uor-redcap.reading.ac.uk/surveys/?s=ABCD1234**]{style="color:teal"}[**&**]{style="color:purple"}[**prolific_pid={{%PROLIFIC_PID%}}&study_id={{%STUDY_ID%}}&session_id={{%SESSION_ID%}}**]{style="color:brown"}
...
You can then use the variables prolific_pid study_id or session_id later on to pipe values to Gorilla or a similar additional service. An example of porting variables out of REDCap is explained at [10.9   External Study Credit Granting with REDCap (REDCap \> SONA)] (please note, the variable names given here may be different, you will need to make sure they are consistent across platforms otherwise you may experience difficulties).
...
-   You can change them by selecting the 'Edit' button ![](images/edit.png){width="36"}

```{=html}
<!-- -->
```
-   You can move them by selecting the 'Move' button ![](images/move.png){width="38"}

-   You can delete them by selecting the 'Delete' button ![](images/delete.png)
...
[Accessing the UoR subscription](https://support.gorilla.sc/support/reference/subscription-FAQ#howdoiaccessmysubscription) (you will need to use the enrolment code: [PCLS_2021]{style="color:red"})
...
You now need to edit the URL so that SONA knows where to find the participant ID on REDCap. On our REDCap Consent form, the participant ID was stored in a field we created called 'ppt_id'. So, we will replace the URL parameter '[survey_code=\[sona_id:value\]]{style="color:brown"}' with '[survey_code=\[ppt_id\]]{style="color:brown"}'.

Your URL should then look something like this:

::: {style="color:brown"}
https://rdg-psychology.sona-systems.com/webstudy_credit.aspx?experiment_id=2532&credit_token=c4dbc01a4b5a4dd8ad3042e00683c855[&survey_code=\[ppt_id\]]{style="background-color:yellow"}
:::
...
Paste the edited SONA URL (the one that looks like our example:

::: {style="color:brown"}
https://rdg-psychology.sona-systems.com/webstudy_credit.aspx?experiment_id=2532&credit_token=c4dbc01a4b5a4dd8ad3042e00683c855[&survey_code=\[ppt_id\]]{style="background-color:yellow"}
:::

) into the 'redirect to a URL' setting under 'Survey Termination Options' within the survey settings for the Information Sheet and Consent Form.
